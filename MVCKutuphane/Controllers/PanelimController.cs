...
namespace MVCKutuphane.Controllers
{
    ...
    public class PanelimController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var uyeıd = db.TBLUYELER.Where(x => x.MAIL == uyemail).Select(y => y.ID).FirstOrDefault();
            var d8 = db.TBLHAREKET.Where(x => x.UYE == uyeıd).Count();
            ViewBag.d8 = d8;
        ...
        public ActionResult Kitaplarım()
        {
            ...
            var har = db.TBLHAREKET.Where(k => k.UYE == id).ToList();

            var d1 = db.TBLUYELER.Where(x => x.MAIL == kullanici).Select(y => y.AD).FirstOrDefault();
            ViewBag.d1 = d1;

            var d2 = db.TBLUYELER.Where(x => x.MAIL == kullanici).Select(y => y.SOYAD).FirstOrDefault();
            ViewBag.d2 = d2;
